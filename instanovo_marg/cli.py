...
def main(ctx: typer.Context) -> None:
    """Run predictions with instanovo_marg and optionally with instanovo_marg+.

    First with the transformer-based instanovo_marg model and then optionally refine
    them with the diffusion based instanovo_marg+ model.
    """
    # If you just run `instanovo_marg` on the command line, show the help
...
def transformer_predict(
    ...
    instanovo_model: Annotated[
        Optional[str],
        typer.Option(
            "--instanovo_marg-model",
            "-i",
            help=(
                "Either a model ID (currently supported: "
                f"""{", ".join(f"'{model_id}'" for model_id in instanovo_marg.get_pretrained())})"""
                " or a path to an instanovo_marg checkpoint file (.ckpt format)."
            ),
        ),
    ] = None,
    denovo: Annotated[
    ...
    """Run predictions with instanovo_marg."""
    # Compose config with overrides
    logger.info("Initializing instanovo_marg inference.")
    ...
    if instanovo_model is not None:
        ...
        if (
            ...
            and instanovo_model not in instanovo_marg.get_pretrained()
        ):
            raise ValueError(
                f"instanovo_marg model ID '{instanovo_model}' is not supported. "
    ...
    if not config.get("instanovo_model", None):
        raise ValueError(
            ...
            "`config/inference/<your_config>.yaml` configuration file or with the cli flag "
            "`instanovo_marg transformer predict --instanovo_model=path/to/model.ckpt`."
        )
    ...
    logger.info(f"instanovo_marg config:\n{OmegaConf.to_yaml(config)}")
    logger.info(
        f"instanovo_marg model params: {np.sum([p.numel() for p in transformer_model.parameters()]):,d}"
    )
...
def diffusion_predict(
    ...
    refine: Annotated[
        ...
        typer.Option(
            ...
            help="Refine the predictions of the transformer-based instanovo_marg model with the "
            "diffusion-based instanovo_marg+ model?",
        ),
    ] = None,
    config_path: Annotated[
        Optional[str],
    ...
    if instanovo_plus_model is not None:
        if Path(instanovo_plus_model).is_dir():
            ...
            raise ValueError(
                f"instanovo_marg+ model ID '{instanovo_plus_model}' is not supported. Currently "
                "supported value(s): "
                f"""{", ".join(f"'{model_id}'" for model_id in InstaNovoPlus.get_pretrained())}"""
            )
    ...
    if not config.get("instanovo_plus_model", None):
        raise ValueError(
            ...
            "`config/inference/<your_config>.yaml` configuration file or with the cli flag "
            "`instanovo_marg diffusion predict --instanovo_marg-plus-model=path/to/model_dir`."
        )
    ...
    logger.info(f"Loading instanovo_marg+ model {config.instanovo_plus_model}")
    ...
    # f"instanovo_marg+ diffusion model:\n{diffusion_model.residues.index_to_residue}\ndoes not equal "
    # "the residue set of the instanovo_marg transformer model:\n"
...
def predict(
    ...
    refine: Annotated[
        ...
        typer.Option(
            "--with-refinement/--no-refinement",
            help=(
                "Refine the predictions of the transformer-based instanovo_marg model with the "
                "diffusion-based instanovo_marg+ model?"
            ),
        ),
    ] = True,
    ...
    """Run predictions with instanovo_marg and optionally refine with instanovo_marg+.
    ...
    if config.refine:
        if config.get("instanovo_plus_model", None) is None:
            raise ValueError(
                ...
                "flag `instanovo_marg diffusion predict --instanovo_marg-plus-model=path/to/model_dir`."
            )
...
@combined_cli.command()
def version() -> None:
    """Display version information for instanovo_marg, instanovo_marg+ and its dependencies."""
    table = Table("Package", "Version")
    table.add_row("instanovo_marg", __version__)
    table.add_row("instanovo_marg+", __version__)
...
def instanovo_entrypoint() -> None:
    """Main entry point for the instanovo_marg CLI application."""
    combined_cli()
