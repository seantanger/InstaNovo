...
from instanovo_marg.constants import ANNOTATED_COLUMN, ANNOTATION_ERROR, DIFFUSION_START_STEP
from instanovo_marg.diffusion.multinomial_diffusion import InstaNovoPlus
from instanovo_marg.inference.diffusion import DiffusionDecoder
from instanovo_marg.transformer.dataset import SpectrumDataset, collate_batch
from instanovo_marg.transformer.predict import _format_time
...
def get_preds(
    ...
    if config.get("refine", False) and config.get("instanovo_predictions_path", None) is None:
        raise ValueError("The instanovo_marg predictions csv path is missing.")
    ...
    if config.get("refine", False):
        logger.info("Loading instanovo_marg predictions.")
        ...
        if id_col not in instanovo_preds_df.columns:
            raise ValueError(
                f"The column '{id_col}' does not exist in one instanovo_marg predictions data. "
                "Set an appropriate 'id_col' value in the config."
            )
        ...
        )  # replace sequence column with instanovo_marg predictions
        sdf.df = sdf.df.drop(config.get("pred_col", "predictions"))

        logger.info("Successfully merged instanovo_marg predictions into input data.")
