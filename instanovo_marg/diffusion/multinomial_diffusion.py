...
from instanovo_marg.diffusion.model import MassSpectrumTransFusion
from instanovo_marg.types import Peptide, ResidueLogProbabilities, TimeStep
        ...
        if os.path.isdir(model_id):
            if all(os.path.exists(os.path.join(model_id, fn)) for fn in expected_files):
                ...
                raise FileNotFoundError(
                    f"instanovo_marg+ model directory {model_id} is missing "
        ...
        if "remote" in model_info:
            ...
            if os.path.isdir(instanovo_plus_model):
                if all(
                    ...
                    raise FileNotFoundError(
                        f"instanovo_marg+ model directory {instanovo_plus_model} is missing the "
